...
class Linphone(threading.Thread):
    ...
    """
    def __init__(self, wxFrame=None, args=()):
    ...
    def setLineStatus(self, lineStatus):
        """
        Sets the status of the subprocess readline()
        """
    ...
    def run(self):
        while self.isrunning():
            ...
            #not 'incoming'
                ...
                wx.PostEvent(self.frame, evt)
                self.setLineStatus('Incoming Call')
                print 'Incoming Call'
            elif (line.find("No active call.") != -1):
                if (self.onCall):
                    self.onCall = False
                self.setLineStatus('None')
