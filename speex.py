...
import sys
try:
    speexDll = CDLL("libspeex.so")
except:
    speexDll = CDLL("libspeex.dll")
...
class Encoder:
    def initialize(self, mode=SPEEX_MODEID_UWB):
        ...
        self.set(SPEEX_SET_VBR, 1)
        self.set(SPEEX_SET_QUALITY, 8)
        self.set(SPEEX_SET_COMPLEXITY, 3)
        self.set(SPEEX_SET_HIGHPASS, 1)
    ...
    def getframesize(self):
        return self.get(SPEEX_GET_FRAME_SIZE)
    ...
    def get(self, request):
        if request in [SPEEX_GET_FRAME_SIZE]:
            tmp = c_int()
            speexDll.speex_encoder_ctl(self.state, request, byref(tmp))
            return tmp.value
        else:
